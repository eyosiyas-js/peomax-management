...
export const EventStep1FORM = () => {
  ...
  const addValues = async () => {
    ...
    for (let key in values) {
      if (
        ...
        (values[key] === '' || values[key] === null || files.length <= 0)
      ) {
        allNotEmpty = false
        break
      }
    ...
    if (allNotEmpty) {
      ...
      await Axiosinstance.post('/api/events/create', formData, {
        headers: {
  ...
  return (
    <form autoComplete="off" noValidate onSubmit={handleSubmit}>
      <Card>
        <CardHeader
          subheader="The information is used to create an Event under this orginization"
          title="Step 1"
        />
        ...
        <Grid sx={{ display: 'flex', justifyContent: 'center', mt: 1 }}>
          <Button
            ...
            variant={selectedButton === 'event' ? 'contained' : 'outlined'}
          ...
          <Button
            sx={{ mx: '10px', width: '100px' }}
            variant={selectedButton === 'program' ? 'contained' : 'outlined'}
        ...
        <CardContent sx={{ pt: 0 }}>
          <Box sx={{ m: -1.5 }}>
              ...
              <Grid xs={12} md={3}>
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <DesktopDatePicker
                    ...
                    shouldDisableDate={
                      values.date ? shouldDisableEndDate : shouldDisableDate
                    }
                    renderInput={(params) => <TextField {...params} />}
                  />
                </LocalizationProvider>
            ...
            <Grid xs={12} md={12} display={'flex'} justifyContent={'center'}>
              {selectedButton === 'event' ? (
                <Button onClick={addValues}>Create Event</Button>
              ) : (
